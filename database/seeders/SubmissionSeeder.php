...
class SubmissionSeeder extends Seeder
{
    public function run()
    {
        ...
        if ($guideline4 && $allUsers->count() > 0) {
            $submission4 = Submission::create([
                'user_id' => $allUsers->first()->id,
                'guideline_id' => $guideline4->id,
                'submission_number' => 'BMKG-SUR-2609-2025-0004',
                'type' => 'pnbp',
                'documents' => [
                    [
                        'index' => 0,
                        'original_name' => 'Surat_Pengantar.pdf',
                        'stored_name' => 'surat_pengantar_' . time() . '.pdf',
                        'path' => 'submissions/pnbp/2025/09/surat_pengantar_' . time() . '.pdf',
                        'size' => 768000,
                        'mime_type' => 'application/pdf',
                        'extension' => 'pdf',
                        'uploaded_at' => Carbon::now()->toISOString()
                ...
                'purpose' => 'Informasi cuaca untuk acara outdoor perusahaan yang akan dilaksanakan minggu depan.',
                'status' => 'payment_pending',
                'created_at' => Carbon::now()->subDays(5),
                'updated_at' => Carbon::now()->subDays(3)
            ]);

            // Create payment for submission4
            $payment4 = Payment::create([
                'submission_id' => $submission4->id,
                'amount' => $guideline4->fee,
                'status' => 'uploaded',
                'payment_proof' => 'payments/2025/09/payment_proof_je9a_' . time() . '.jpg',
                'payment_method' => 'Transfer Bank',
                'payment_reference' => 'JE9A' . time(),
                'paid_at' => Carbon::now()->subDays(2)
            ]);

            // Add histories for submission4
            $submission4->logHistory(
                'submitted',
                'user',
                $allUsers->first()->id,
                'Pengajuan Surat/Data Disubmit',
                'User mengajukan ' . $guideline4->title
            );
            ...
            $submission4->logHistory(
                ...
                null,
                'Menunggu Pembayaran',
                'Pembayaran sebesar ' . number_format($guideline4->fee, 0, ',', '.') . ' diperlukan untuk melanjutkan proses'
            );
            ...
            $submission4->logHistory(
                ...
                $allUsers->first()->id,
                'Bukti Pembayaran Diunggah',
                'User mengunggah bukti pembayaran melalui Transfer Bank dengan referensi: JE9A' . time()
            );
        }
